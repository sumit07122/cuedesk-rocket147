...
export const BillingView: React.FC<BillingViewProps> = ({
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Receipt No', 'Date & Time', 'Customer Name', 'Phone', 'Table', 'Duration (min)', 'Table Fee', 'Food Fee', 'Extra Fee', 'Discount', 'Tax', 'Grand Total', 'Amount Paid', 'Balance', 'Payment Method', 'Status'];
    ...
    const rows = history.map((item) => [
      ...
      (item.extraFee || 0).toFixed(2),
      item.discountAmount.toFixed(2),
      item.taxAmount.toFixed(2),
      item.grandTotal.toFixed(2),
      (item.amountPaid ?? item.grandTotal).toFixed(2),
    ...
    link.setAttribute('download', `Rocket147_Payments_Export_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div className="flex flex-col gap-6 p-4 sm:p-6 max-w-7xl mx-auto w-full pb-24 lg:pb-8">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-neutral-100 gap-2">
                      <div>
                        <span className="text-xs uppercase font-bold text-neutral-400 tracking-wider">
                          Rocket 147 Official Session Statement
                        </span>
                        ...
                        <div className="p-4 rounded-2xl bg-neutral-900 text-white flex items-center justify-between gap-4 mt-2 shadow-md">
                          <div>
                            <span className="text-[10px] uppercase tracking-wider text-neutral-400 block font-semibold">
                              Rocket 147 Instant UPI QR Code
                            </span>
      ...
      {activeTab === 'audit' && (
        <div className="bg-white rounded-3xl border border-neutral-200 p-6 flex flex-col gap-4 shadow-xs">
          <div>
            <h3 className="text-base font-bold text-neutral-900">Rocket 147 Security & Operational Audit Log</h3>
      ...
      {activeTab === 'eod' && (() => {
        ...
        const handleExportEOD = () => {
          const rows: (string | number)[][] = [
            ['END-OF-DAY REPORT', config?.clubName || 'Rocket 147'],
          ...
          link.setAttribute('download', `Rocket147_EOD_Report_${todayKey}.csv`);
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
        };
